test_that("rt_register detects an NCT number with registration context", {
  article_with_nct <- c(
  ...
  f_with <- tempfile(pattern = "PMID1-", fileext = ".txt")
  f_without <- tempfile(pattern = "PMID2-", fileext = ".txt")
  writeLines(c("Methods", article_with_nct), f_with)
  writeLines(c("Methods", article_without_nct), f_without)

  expect_true(rtransparency::rt_register(f_with)$is_register_pred,
              info = "Should detect NCT number with registration context")
  expect_false(rtransparency::rt_register(f_without)$is_register_pred,
               info = "Should not flag text without NCT")
})


test_that(".which_prospero_1 detects PROSPERO registrations", {
  ...
  idx_found <- rtransparency:::.which_prospero_1(article_with_prospero)
  expect_true(length(idx_found) > 0, info = "Should detect PROSPERO registration")

  idx_empty <- rtransparency:::.which_prospero_1(article_without_prospero)
  expect_equal(length(idx_empty), 0, info = "Should not flag text without PROSPERO ID")
})


test_that(".which_isrctn_1 detects ISRCTN registrations", {
...
test_that("validation registry helpers detect flexible CT, OSF and blinded PROSPERO text", {
  expect_true(length(rtransparency:::.which_ct_4(
  ...
  expect_true(length(rtransparency:::.which_osf_preregistered_1(
    "This work was pre-registered at: https://osf.io/gzh2j/."
  )) > 0)
  expect_true(length(rtransparency:::.which_osf_preregistered_1(
    "This study was pre-registered on the Open Science Framework (OSF)."
  )) > 0)
  expect_true(length(rtransparency:::.which_prospero_redacted_1(
...
test_that("registry helpers return integer(0) for empty/non-matching input", {
  ...
  expect_equal(rtransparency:::.which_isrctn_1(empty), integer(0))
  expect_equal(rtransparency:::.which_anzctr_1(empty), integer(0))
  expect_equal(rtransparency:::.which_drks_1(empty), integer(0))
  expect_equal(rtransparency:::.which_irct_1(non_matching), integer(0))
  expect_equal(rtransparency:::.which_umin_1(non_matching), integer(0))
  expect_equal(rtransparency:::.which_chictr_1(non_matching), integer(0))
  expect_equal(rtransparency:::.which_inplasy_1(non_matching), integer(0))
  expect_equal(rtransparency:::.which_osf_protocol_1(non_matching), integer(0))
  expect_equal(rtransparency:::.which_osf_preregistered_1(non_matching), integer(0))
  expect_equal(rtransparency:::.which_prospero_redacted_1(non_matching), integer(0))
})
